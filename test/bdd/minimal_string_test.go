...
func TestNewBddFromMinimalString(t *testing.T) {
	root, err := bdd.NewBddFromMinimalString(minimalBddString)
	if err != nil {
		t.Fatalf("error creating bdd from minimal string: %v", err)
	}
