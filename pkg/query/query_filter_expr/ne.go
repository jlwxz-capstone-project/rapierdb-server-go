...
func (e *NeExpr) MarshalJSON() ([]byte, error) {
...
func (e *NeExpr) UnmarshalJSON(data []byte) error {
	var s SerializedQueryFilterExpr
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}
	if s.Type != ExprTypeNe {
		return fmt.Errorf("expected NE expression, got %s", s.Type)
	}
	if s.O1 == nil || s.O2 == nil {
		return fmt.Errorf("missing operands for NE expression")
	}
